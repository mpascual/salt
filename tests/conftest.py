...
def pytest_saltfactories_generate_default_master_configuration(
    ...
    if master_id == "master":
        with salt.utils.files.fopen(os.path.join(RUNTIME_VARS.CONF_DIR, 'master')) as rfh:
            ...
            tests_known_hosts_file = factories_manager.root_dir.join('salt_ssh_known_hosts').strpath
        ...
        with salt.utils.files.fopen(os.path.join(RUNTIME_VARS.CONF_DIR, 'syndic_master')) as rfh:
            ...
            opts['hosts.file'] = factories_manager.root_dir.join('hosts').strpath
            opts['aliases.file'] = factories_manager.root_dir.join('aliases').strpath
            opts['transport'] = request.config.getoption('--transport')
...
def pytest_saltfactories_master_configuration_overrides(
    ...
    extension_modules_path = factories_manager.root_dir.join('extension_modules').strpath
    ...
    autosign_file_path = factories_manager.root_dir.join('autosign_file').strpath
    shutil.copyfile(
